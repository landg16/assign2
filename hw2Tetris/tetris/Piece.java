...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		width = 0;
		height = 0;
		computeWidthHeight(points);
		computeSkirt(points);
	}
	
	private void computeWidthHeight(TPoint[] points){
		for(int i = 0; i<points.length; i++){
			if(width-1<points[i].x) width = points[i].x + 1;
			if(height-1<points[i].y) height = points[i].y + 1;
		}
	}
	...
	private void computeSkirt(TPoint[] points){
		...
		Arrays.fill(skirt, -1);
		for(int i = 0; i<points.length; i++){
			if(skirt[points[i].x]>points[i].y || skirt[points[i].x]==-1)
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] tmp = new TPoint[body.length];
		for(int i = 0; i<body.length; i++){
			tmp[i] = new TPoint(height - body[i].y - 1, body[i].x);
		}
		Piece pc = new Piece(tmp);
		return pc;
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		TPoint[] tmpPoint = other.getBody();
		
		for(int i = 0; i<tmpPoint.length; i++){
			boolean isFound = false;
			for(int j = 0; j<body.length; j++){
				if(tmpPoint[i].equals(body[j]))  isFound = true;
			}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece tmp = root;
		while(true) {
			tmp.next = tmp.computeNextRotation();
			if(tmp.next.equals(root)) break;
			tmp = tmp.next;
		}
		tmp.next = root;
		return root;
